...
function getData() {
	fetch("https://iesdesigner.eu/wordpress/wp-json/wp/v2/film?_embed").then(res => res.json()).then(handleData)
}
...
function showPost(post) {
	...
	const logline = postCopy.querySelector(".logline");
	logline.innerHTML = post.content.rendered;
	...
	postCopy.querySelector(".trailer_modal").addEventListener("click",
		() => {
			fetch(`https://iesdesigner.eu/wordpress/wp-json/wp/v2/film${post.id}`).then(res => res.json()).then(seetrailer);
		});
	...
	function appendIntitle() {
		if (post.id == id) document.querySelector("#post-inside").appendChild(postCopy);
	}
	appendIntitle()
}